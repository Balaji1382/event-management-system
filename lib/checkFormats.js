...
const checkDateFormat = (date) => {
    ...
    // if date has only three fields separated by '-', and if length of each field match their format
    if(dateArr.length !== 3 || dateArr[0].length !== 4 || dateArr[1].length !== 2 || dateArr[2].length !== 2){
        return false;
    }
    // if months are between 1 and 12
    if(!Number(dateArr[1]) || (Number(dateArr[1]) > 12) || (Number(dateArr[1]) < 1)){
        return false;
    }
    // if all dates of a month is between 1 and 31
    if(!Number(dateArr[2]) || (Number(dateArr[2]) > 31) || (Number(dateArr[2]) < 1)){
        return false;
    }
    // if february, number of days should be limited to 29
    if((Number(dateArr[1]) === 2) && (Number(dateArr[2]) > 29)){
        return false;
    }
    // if not a leap year, february should have 28 days
...
const checkLongitudeFormat = longitude => {
    ...
    // if number part returns NaN or out of range
    if(isNaN(Number(arr[0])) || Number(arr[0]) < -180 || Number(arr[0]) > 180){
        return false;
    }
    if(arr[1]){
        // if decimal part contains signs
        if(arr[1][0] === '+' || arr[1][0] === '-'){
            return false;
        }
        // if decimal part returns NaN
